...
Class Record extends CI_Controller
{
	...
	function new_record($username = NULL,$agent_id = NULL,$phonenumber = NULL,$first_name = NULL,$last_name = NULL,$utility = NULL,$email = NULL,$acct1 = NULL,$acct2 = NULL,$acct3 = NULL,$acct4 = NULL,$acct5= NULL,$work_group = NULL,$promo_offered = NULL,$source = NULL,$call_dispo = NULL,$call_record_id = NULL,$call_origin = NULL,$question = NULL,$web_dispo = NULL,$remarks = NULL)
	{
		...
		if(isset($_POST['work_group']) && $_POST['work_group'] =='New Sales' &&  isset($_POST['save'])  && $_POST['save']=='Save All')
			{
				...
				$this->form_validation->set_rules('acct1','acct1','trim|xss_clean|required');
				...
				$this->form_validation->set_rules('call_origin','call_origin','trim|xss_clean|required');
				$this->form_validation->set_rules('promo_offered','promo_offered','trim||xss_clean|required');
				$this->form_validation->set_rules('source','source','trim|xss_clean|required');
				$this->form_validation->set_rules('call_dispo','call_dispo','trim||xss_clean|required');
				$this->form_validation->set_rules('call_record_id','call_record_id','trim|xss_clean|required');
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$this->load->view('master_view', array('page' => 'new_view', 'data' => $this->load_dropdowns($phonenumber, $this->input->post('work_group'))));
		}
	}else
				{
	...
	{
		$this->load->library('form_validation');
		if(isset($_POST['work_group']) && $_POST['work_group'] =='Sale'  &&  isset($_POST['save'])  && $_POST['save']=='Save All' )
				{
					$this->form_validation->set_rules('first_name','first_name','trim|xss_clean');
					$this->form_validation->set_rules('last_name','last_name','trim||xss_clean');
					$this->form_validation->set_rules('utility','utility','trim|xss_clean|required');
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$this->load->view('master_view', array('page' => 'reload_view', 'data' => $this->load_dropdowns($phonenumber,$this->input->post('work_group'))));
			}
		}else
				{
					redirect('/user/login/', 'refresh');
